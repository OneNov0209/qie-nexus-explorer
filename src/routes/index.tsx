...
function HomePage() {
  ...
  const stats_cards = [
    ...
    { label: "Total Supply", value: stats?.supply ? formatQIE(stats.supply, 0) : "—", icon: <Coins className="w-5 h-5 text-white" />, color: "from-amber-500 to-orange-500", shadow: "shadow-amber-500/20" },
    { label: "Staking Ratio", value: stats?.stakingRatio ? `${stats.stakingRatio.toFixed(1)}%` : "—", icon: <Layers className="w-5 h-5 text-white" />, color: "from-emerald-500 to-teal-500", shadow: "shadow-emerald-500/20" },
  ];
                ...
                {stats?.topValidators?.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={stats.topValidators} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
                      <defs><linearGradient id="homeAreaGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#8B5CF6" stopOpacity={0.4} /><stop offset="95%" stopColor="#D946EF" stopOpacity={0} /></linearGradient></defs>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} className="relative rounded-2xl border-2 border-border/40 bg-card/40 backdrop-blur-sm p-5 hover:border-violet-500/20 transition-all duration-300 max-w-6xl mx-auto shadow-lg hover:shadow-xl">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {price?.usd_24h_change && <span className={`text-xs font-medium ${price.usd_24h_change > 0 ? 'text-emerald-400' : 'text-red-400'}`}>{price.usd_24h_change > 0 ? '↑' : '↓'} {Math.abs(price.usd_24h_change).toFixed(2)}%</span>}
              ...
              <div className="flex items-center gap-1 bg-muted/30 rounded-xl p-1 border border-border/40">
                {PERIODS.map((p) => (
                  <button key={p.key} onClick={() => setPeriod(p.key)} className={`px-3 py-1.5 text-xs rounded-lg transition-all ${period === p.key ? "bg-violet-500 text-white shadow-md" : "text-muted-foreground hover:text-foreground"}`}>{p.label}</button>
                ))}
              </div>
            </div>
            <div className="h-64">
      ...
      <section className="relative py-16 md:py-24">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4"><span className="bg-gradient-to-r from-violet-400 to-cyan-400 bg-clip-text text-transparent">Everything You Need</span></h2>
      ...
      <footer className="mt-auto border-t-2 border-border/60 bg-background/40 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-2 md:grid-cols-5 gap-8">
          <div className="col-span-2 md:col-span-1">
            <div className="flex items-center gap-3">
              <img src={NETWORK.logo} className="w-10 h-10 rounded-full ring-2 ring-primary/40 shadow-lg" alt="" />
              <div><div className="font-semibold gradient-text text-lg">QIE Explorer</div><div className="text-xs text-muted-foreground">Mainnet · Chain {NETWORK.chainId}</div></div>
            </div>
            <p className="text-xs text-muted-foreground mt-4 leading-relaxed">Hybrid Cosmos + EVM block explorer for the QIE ecosystem.</p>
            <a href={`mailto:${FOOTER_LINKS.email}`} className="mt-4 inline-flex items-center gap-2 text-xs text-muted-foreground hover:text-primary"><Mail className="w-3.5 h-3.5" /> {FOOTER_LINKS.email}</a>
            <p className="text-xs text-muted-foreground mt-3">Powered by{" "}<a href="https://onenov.xyz" target="_blank" rel="noreferrer" className="text-violet-400 hover:text-violet-300 transition-colors font-medium">OneNov</a></p>
          </div>
        ...
        <div className="border-t-2 border-border/60 py-4 text-center text-xs text-muted-foreground">© {new Date().getFullYear()} QIE Blockchain. All rights reserved.</div>
      </footer>
    </div>
  );
}
...
function ProgressBar({ label, value, max, color, textColor, suffix }: { label: string; value: number; max: number; color: string; textColor: string; suffix?: string }) {
  ...
  return (
    <div>
      <div className="flex justify-between text-xs mb-1"><span className="text-muted-foreground">{label}</span><span className={`font-bold ${textColor}`}>{value}{suffix || `/${max}`}</span></div>
      <div className="h-1.5 rounded-full bg-muted/50 overflow-hidden border border-border/30 shadow-inner">
        <div className={`h-full bg-gradient-to-r ${color} rounded-full transition-all duration-500 shadow-sm`} style={{ width: `${Math.min(pct, 100)}%` }} />
      </div>
    </div>
  );
}
