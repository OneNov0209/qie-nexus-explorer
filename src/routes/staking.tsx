...
import { ExternalLink, Gift, Loader2, Layers, Coins, Search, Filter, User, Shield, AlertTriangle, CheckCircle, Clock, TrendingUp, Percent, ChevronRight } from "lucide-react";
...
const VALIDATOR_AVATARS: Record<string, string> = {
  // Keybase identity → avatar URL akan di-fetch dari Keybase API
};

function getValidatorAvatar(identity: string): string {
  if (!identity) return "";
            ...
            {modal?.type === "Redelegate" && (
              <div>
                ...
                >
                  ...
                  {list
                    .filter((x: any) => x.operator_address !== modal.val.operator_address && x.status === "BOND_STATUS_BONDED")
                    .map((x: any) => (
                      <option key={x.operator_address} value={x.operator_address}>{x.description?.moniker}</option>
                    ))}
                </select>
              </div>
            )}
