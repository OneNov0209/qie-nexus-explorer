...
function makeRegistry() {
  ...
  // Ethermint pubkey has same wire format as cosmos secp256k1 PubKey (single `key` bytes field)
...
async function getSigner() {
  ...
  const provider = w.keplr ?? w.leap;
  if (!provider) throw new Error("Keplr or Leap wallet not detected");
  await provider.enable(NETWORK.cosmosChainId);
  // For eth-key chains, prefer the direct signer; Keplr exposes it via getOfflineSigner
...
function coin(amount: string | number) {
  // amount is already in micro-units (aqie). Allow user to pass whole QIE and convert.
...
export function toMicro(qie: string | number): string {
  ...
  return BigInt(Math.floor(n * 1e6)).toString() + "000000000000"; // 1e18 total
}
...
async function broadcast(msgs: any[], memo = ""): Promise<DeliverTxResponse> {
  ...
  // signAndBroadcast handles encoding; pass sender from accounts[0]
